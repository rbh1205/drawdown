<script lang="ts">
	...
	onMount(async () => {
		...
		const [bag, allRoasters, allOrigins] = await Promise.all([
			getBag(id),
			getAllRoasters(),
			getAllOrigins()
		]);

		if (!bag) { notFound = true; loading = false; return; }
		...
		processDetail = bag.processDetail ?? '';
		selectedRoasterId = bag.roasterId;
		selectedOriginId = bag.originId;
		variety = bag.variety ?? '';
		altitude = bag.altitude ?? '';
		roastDate = bag.roastDate ?? '';

		if (variety || altitude || processDetail) showOptional = true;
	...
	async function handleSubmit(e: Event) {
		...
		const existingBag = await getBag(bagId);
		if (!existingBag) { saving = false; return; }
		...
		const updated: Bag = {
			...
			originId,
			processingMethod,
			processDetail: processDetail.trim() || undefined,
